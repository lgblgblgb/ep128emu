...
static int sdext_load_flash ( const char *img_fn, Uint8 *target, int size)
{
	FILE *f = fopen(img_fn, "rb");
	if (f == NULL) {
		fprintf(stderr, "SD card FLASH file \"%s\" cannot be open: %s. You can use Xep128 but SD card access won't work!\n", img_fn, strerror(errno));
		return 1;
	}
	if (fread(target, 1, size, f) != size) {
		fprintf(stderr, "SD card FLASH file \"%s\" cannot be read for %d bytes: %s\n", img_fn, size, strerror(errno));
		fclose(f);
		return 1;
	}
	fclose(f);
	printf("SDEXT: wow, FLASH is loaded :-)\n");
	return 0;
}
...
{
	if (!need_init) {
		printf("SDEXT: init already issued, skipping\n");
		return;
	}
	need_init = 0;
	if (sdf != NULL)
		fclose(sdf);
	if (sdf == NULL) {
		sdf = fopen(SDCARD_IMG_FN, "rb");
	}
	// sdf = open_emu_file(SDCARD_IMG_FN, "rb", sdimg_path);
	if (sdf == NULL) {
		fprintf(stderr, "SD card image file \"%s\" cannot be open: %s. You can use Xep128 but SD card access won't work!\n", SDCARD_IMG_FN, strerror(errno));
		//*sdimg_path = 0;
		sdfno = -1;
	} else {
		if (sdfno == -1) sdfno = fileno(sdf);
		printf("SDEXT: cool, SD image file is opened as %s with fileno of %d\n", SDCARD_IMG_FN, sdfno);
	}
...
static int safe_read ( int fd, Uint8 *buffer, int size )
{
	int all = 0;
	while (size) {
...
#ifdef DEBUG_SDEXT
	printf("SDEXT: REGIO: fread, begin ...\n");
#endif
	//ret = fread(_buffer + 2, 1, 512, sdf);
	ret = safe_read(sdfno, _buffer + 2, 512);
#ifdef DEBUG_SDEXT
	printf("SDEXT: REGIO: fread retval = %d from fdno = %d\n", ret, sdfno);
	if (ret == -1) {
		printf("SDEXT: REGIO: failed read, see next msg ...\n");
		printf("SDEXT: REGIO: READ ERROR: %s\n", strerror(errno));
	}
#endif
	_buffer[512 + 2] = 0; // CRC
	_buffer[512 + 3] = 0; // CRC
...
Uint8 sdext_read_cart_p3 ( Uint16 addr )
{
	...
	fprintf(stderr, "SDEXT: FATAL, control should not get here\n");
	exit(1);
...
void sdext_write_cart_p3 ( Uint16 addr, Uint8 data )
{
	...
	printf("SDEXT: write P3 @ %04X with value %02X\n", addr & 0x3FFF, data);
#endif
